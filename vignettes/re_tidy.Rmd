...
summary_rr_lrr <- summary_rr_lrr |>
  mutate(phase = str_to_lower(phase))  # 使用 stringr::str_to_lower 统一小写
...
summary_long <- summary_rr_lrr |>
  ...
  mutate(
    ...
    phase = factor(phase, levels = c("early", "mid", "late"))  # 指定顺序
  )
...
For the comparison of CO2_8 vs CO2_9 (blue line): in the early stage, Mean LRR is at a relatively high level (around 0.06); as the stage progresses to the mid stage, Mean LRR decreases (to around 0.025); and in the late stage, it decreases further, even approaching to less than 0 (around -0.005), showing a clear trend of decreasing with advancing stage. The trend of decreasing with the advancement of the stage is obvious.
- For the comparison of CO2_6 vs CO2_7 (red line): in the early stage, the Mean LRR is low and positive (~0.01); in the mid stage, there is a small increase (to ~0.018); however, in the late stage, there is a significant decrease to negative (~-0.03), with a clear decreasing trend in the late stage as a whole.
